...
namespace Hjson {
		...
		Parser* p, const CommentInfo& ciA, const CommentInfo& ciB)
	{
		if (ciA.hasComment && ciB.hasComment) {
			(val.*fp)(std::string(p->data + ciA.cmStart, p->data + ciA.cmEnd) + std::string(p->data + ciB.cmStart, p->data + ciB.cmEnd));
		}
		else {
	...
	static std::string _errAt(Parser* p, const std::string& message)
	{
		...
		return message + " at line " + std::to_string(line) + "," + std::to_string(col) + " >>> " + std::string((char*)p->data + p->at - col, samEnd);
	}
		...
		for (;;) {
			if (p->ch == ':') {
				...
				if (_isPunctuatorChar(p->ch)) {
					throw syntax_error(_errAt(p, std::string("Found '") + (char)p->ch + std::string("' where a key name was expected (check your syntax or use quotes if the key name includes {}[],: or whitespace)")));
				}
				keyEnd = p->at;
			}
			_next(p);
	...
	{
		if (_isPunctuatorChar(p->ch)) {
			throw syntax_error(_errAt(p, std::string("Found a punctuator character '") + (char)p->ch + std::string("' when expecting a quoteless string (check your syntax)")));
		}
		...
		for (;;) {
			...
			if (isEol || p->ch == ',' || p->ch == '}' || p->ch == ']' || p->ch == '#' || (p->ch == '/' && (_peek(p, 0) == '/' || _peek(p, 0) == '*'))) {
	...
	static Value _readObject(Parser* p, bool withoutBraces)
	{
		...
		while (p->ch > 0) {
			...
			auto ciKey = _white(p);
			if (p->ch != ':') {
				throw syntax_error(_errAt(p, std::string("Expected ':' instead of '") + (char)(p->ch) + "'"));
			}
			_next(p);
			...
			if (!elem.get_comment_before().empty()) {
				elem.set_comment_key(elem.get_comment_key() + elem.get_comment_before());

...
namespace Hjson {
		...
		bool isRootObject, bool hasCommentAfter)
	{
		if (value.size() == 0) {
			*e->os << separator << "\"\"";
		...
		else if (e->opt.quoteAlways || std::regex_search(value, e->needsQuotes) || startsWithNumber(value.c_str(), value.size()) || std::regex_search(value, e->startsWithKeyword) || hasCommentAfter) {
			...
			else if (!e->opt.quoteAlways && !std::regex_search(value, e->needsEscapeML) && !isRootObject) {
	...
	{
		const char* separator = ((isObjElement && (!e->opt.comments || value.get_comment_key().empty())) ? " " : "");
